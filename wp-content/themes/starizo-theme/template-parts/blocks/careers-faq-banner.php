...
$button_link = get_sub_field('button_link') ?: site_url('/contact');
...
if (empty($faqs_list)) {
    $faqs_list = array(
        ...
        array(
            ...
            'answer'   => 'Yes, technical specifications, safety data sheets, and certificates of analysis are available upon request.',
        ),
        array(
            'question' => 'Do you support international supply?',
            'answer'   => 'Yes, we deliver ingredients to major hubs globally through our international distribution network.',
        ),
        array(
            'question' => 'Can STARIZO support product development?',
            'answer'   => 'Yes, our team collaborates from formulation to pilot batch validation.',
        ),
        array(
            'question' => 'Is there a minimum order requirement?',
            'answer'   => 'Minimum order quantities vary by product category. Contact our sales team for details.',
        ),
    );
}
?>

<section class="w-full py-16 md:py-24 bg-[#FDFBF3] border-t border-amber-100/60">
  ...
  <div class="max-w-7xl mx-auto px-6 md:px-8 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-stretch">
        ...
        ?>
          <div class="bg-white border border-gray-100 rounded-3xl py-4 px-6 md:py-[18px] md:px-8 shadow-sm faq-item cursor-pointer">
            <div class="flex justify-between items-center gap-4">
              <h4 class="font-montserrat font-semibold text-[16px] text-black leading-[24px] tracking-[0em]"><?php echo esc_html($faq['question']); ?></h4>
              <?php if ($is_expanded) : ?>
                <div class="w-5 h-[2px] bg-[#5D3700] rounded-full faq-icon"></div>
              <?php else : ?>
                <div class="w-5 h-5 flex items-center justify-center relative text-[#FF8D00] faq-icon">
            ...
            <?php if (!empty($faq['answer'])) : ?>
              <p class="mt-3 text-[15px] text-gray-600 leading-[1.6] max-w-xl faq-answer <?php echo $is_expanded ? '' : 'hidden'; ?>">
    ...
    item.addEventListener('click', function() {
      ...
      if (answer) {
        if (answer.classList.contains('hidden')) {
          ...
          iconContainer.className = 'w-5 h-[2px] bg-[#5D3700] rounded-full faq-icon';
          iconContainer.innerHTML = '';
        } else {
          answer.classList.add('hidden');
          iconContainer.className = 'w-5 h-5 flex items-center justify-center relative text-[#FF8D00] faq-icon';
