...
function starizo_register_post_types() {
	...
	// Add Order Column to Product Admin List
	add_filter('manage_product_posts_columns', function($columns) {
		$columns['menu_order'] = __('Order', 'starizo-theme');
		return $columns;
	});
...
/**
 * Add Duplicate action link for posts, pages, and custom post types
 */
function starizo_duplicate_post_link( $actions, $post ) {
    if ( current_user_can( 'edit_posts' ) ) {
        $actions['duplicate'] = '<a href="' . wp_nonce_url( 'admin.php?action=starizo_duplicate_post_as_draft&post=' . $post->ID, basename( __FILE__ ), 'duplicate_nonce' ) . '" title="Duplicate this item" rel="permalink">Duplicate</a>';
    }
    return $actions;
}
add_filter( 'post_row_actions', 'starizo_duplicate_post_link', 10, 2 );
add_filter( 'page_row_actions', 'starizo_duplicate_post_link', 10, 2 );

/**
 * Handle the duplication of a post
 */
function starizo_duplicate_post_as_draft() {
    global $wpdb;
    
    if ( ! ( isset( $_GET['post'] ) || isset( $_POST['post'] ) || ( isset( $_REQUEST['action'] ) && 'starizo_duplicate_post_as_draft' == $_REQUEST['action'] ) ) ) {
...
add_action( 'admin_action_starizo_duplicate_post_as_draft', 'starizo_duplicate_post_as_draft' );
